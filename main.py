...
if FLAGS.mode == 'inference':
    ...
    paddings = tf.compat.v1.constant([[0,0], [0,oh], [0,ow], [0,0]])
    print("input shape:", input_shape)
    print("output shape:", output_shape)
    
    # build the graph
    inputs_raw = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=input_shape, name='inputs_raw')
    
    pre_inputs = tf.compat.v1.Variable(tf.compat.v1.zeros(input_shape), trainable=False, name='pre_inputs')
    pre_gen = tf.compat.v1.Variable(tf.compat.v1.zeros(output_shape), trainable=False, name='pre_gen')
    pre_warp = tf.compat.v1.Variable(tf.compat.v1.zeros(output_shape), trainable=False, name='pre_warp')
    
    transpose_pre = tf.compat.v1.space_to_depth(pre_warp, 4)
    inputs_all = tf.compat.v1.concat( (inputs_raw, transpose_pre), axis = -1)
    ...
    inputs_frames = tf.compat.v1.concat( (pre_inputs, inputs_raw), axis = -1)
    with tf.compat.v1.variable_scope('fnet'):
        gen_flow_lr = fnet( inputs_frames, reuse=False)
        gen_flow_lr = tf.compat.v1.pad(gen_flow_lr, paddings, "SYMMETRIC") 
...
# The training mode
    ...
    useValidat = tf.compat.v1.placeholder_with_default( tf.compat.v1.constant(False, dtype=tf.compat.v1.bool), shape=() )
    ...
    config = tf.compat.v1.ConfigProto()
    config.gpu_options.allow_growth = True
    # init_op = tf.compat.v1.initialize_all_variables() # MonitoredTrainingSession will initialize automatically
