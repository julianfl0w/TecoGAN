...
def preprocessLR(image):
    with tf.compat.v1.name_scope("preprocessLR"):
        return tf.compat.v1.identity(image)
    ...
    with tf.compat.v1.variable_scope(scope):
        if use_bias:
            return slim.conv2d_transpose(batch_input, output_channel, [kernel, kernel], stride, 'SAME', data_format='NHWC',
                            activation_fn=None, weights_initializer=tf.initializers.GlorotNormal())
        else:
            return slim.conv2d_transpose(batch_input, output_channel, [kernel, kernel], stride, 'SAME', data_format='NHWC',
                            activation_fn=None, weights_initializer=tf.initializers.GlorotNormal(),
...
def prelu_tf(inputs, name='Prelu'):
    with tf.compat.v1.variable_scope(name):
        alphas = tf.compat.v1.get_variable('alpha', inputs.get_shape()[-1], initializer=tf.compat.v1.zeros_initializer(), \
            collections=[tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, tf.compat.v1.GraphKeys.TRAINABLE_VARIABLES, tf.compat.v1.GraphKeys.MODEL_VARIABLES ],dtype=tf.compat.v1.float32)
    pos = tf.compat.v1.nn.relu(inputs)
...
def denselayer(inputs, output_size):
    ...
    denseLayer = tf.compat.v1.layers.Dense(output_size, activation=None, kernel_initializer=tf.initializers.GlorotNormal())
    ...
    #output = tf.compat.v1.layers.dense(inputs, output_size, activation=None, kernel_initializer=tf.initializers.GlorotNormal())
    
    return output

# The implementation of PixelShuffler
def pixelShuffler(inputs, scale=2):
    size = tf.compat.v1.shape(inputs)
    batch_size = size[0]
    h = size[1]
    w = size[2]
    ...
    input_split = tf.compat.v1.split(inputs, channel_target, axis=3)
    output = tf.compat.v1.concat([phaseShift(x, scale, shape_1, shape_2) for x in input_split], axis=3)
...
def upscale_four(inputs, scope='upscale_four'): # mimic the tensorflow bilinear-upscaling for a fix ratio of 4
    with tf.compat.v1.variable_scope(scope):
        ...
        p_inputs = tf.compat.v1.concat( (inputs, inputs[:,-1:,:,:] ), axis = 1) # pad bottom
        p_inputs = tf.compat.v1.concat( (p_inputs, p_inputs[:,:,-1:,:] ), axis = 2) # pad right
        ...
        hi_res =  tf.compat.v1.stack( hi_res_array, axis = 3 ) # shape (b,h,w,16,c)
        hi_res_reshape = tf.compat.v1.reshape( hi_res, (b, h, w, 4, 4, c) )
        hi_res_reshape = tf.compat.v1.transpose( hi_res_reshape, (0,1,3,2,4,5) )
        hi_res_reshape = tf.compat.v1.reshape( hi_res_reshape, (b, h*4, w*4, c) )
...
def bicubic_four(inputs, scope='bicubic_four'): 
    '''
        equivalent to tf.compat.v1.image.resize_bicubic( inputs, (h*4, w*4) ) for a fix ratio of 4 FOR API <=1.13
        For API 2.0, tf.compat.v1.image.resize_bicubic will be different, old version is tf.compat.v1.compat.v1.image.resize_bicubic
    ...
    with tf.compat.v1.variable_scope(scope):
        ...
        p_inputs = tf.compat.v1.concat( (inputs[:,:1,:,:],   inputs)  , axis = 1) # pad top 
        p_inputs = tf.compat.v1.concat( (p_inputs[:,:,:1,:], p_inputs), axis = 2) # pad left
        p_inputs = tf.compat.v1.concat( (p_inputs, p_inputs[:,-1:,:,:], p_inputs[:,-1:,:,:]), axis = 1) # pad bottom
        p_inputs = tf.compat.v1.concat( (p_inputs, p_inputs[:,:,-1:,:], p_inputs[:,:,-1:,:]), axis = 2) # pad right
        ...
        hi_res_y =  tf.compat.v1.stack( hi_res_array, axis = 2 ) # shape (b,h,4,w,c)
        hi_res_y = tf.compat.v1.reshape( hi_res_y, (b, h*4, w+3, c) )
        ...
        hi_res =  tf.compat.v1.stack( hi_res_array, axis = 3 ) # shape (b,h*4,w,4,c)
        hi_res = tf.compat.v1.reshape( hi_res, (b, h*4, w*4, c) )
        
    return hi_res

def phaseShift(inputs, scale, shape_1, shape_2):
    # Tackle the condition when the batch is None
    X = tf.compat.v1.reshape(inputs, shape_1)
    X = tf.compat.v1.transpose(X, [0, 1, 3, 2, 4])

    return tf.compat.v1.reshape(X, shape_2)

# The random flip operation used for loading examples of one batch
def random_flip_batch(input, decision):
    f1 = tf.compat.v1.identity(input)
    f2 = tf.compat.v1.image.flip_left_right(input)
    output = tf.compat.v1.where(tf.compat.v1.less(decision, 0.5), f2, f1)
...
def random_flip(input, decision):
    f1 = tf.compat.v1.identity(input)
    f2 = tf.compat.v1.image.flip_left_right(input)
    output = tf.compat.v1.cond(tf.compat.v1.less(decision, 0.5), lambda: f2, lambda: f1)
...
def compute_psnr(ref, target):
    ref = tf.compat.v1.cast(ref, tf.compat.v1.float32)
    target = tf.compat.v1.cast(target, tf.compat.v1.float32)
    diff = target - ref
    sqr = tf.compat.v1.multiply(diff, diff)
    err = tf.compat.v1.reduce_sum(sqr)
    v = tf.compat.v1.shape(diff)[0] * tf.compat.v1.shape(diff)[1] * tf.compat.v1.shape(diff)[2] * tf.compat.v1.shape(diff)[3]
    mse = err / tf.compat.v1.cast(v, tf.compat.v1.float32)
    psnr = 10. * (tf.compat.v1.log(255. * 255. / mse) / tf.compat.v1.log(10.))
...
def tf_data_gaussDownby4( HRdata, sigma = 1.5 ):
    ...
    with tf.compat.v1.device('/gpu:0'):
        fix_gkern = tf.compat.v1.constant( gau_wei, dtype = tf.compat.v1.float32, shape = [k_w, k_w, 3, 3], name='gauss_blurWeights' )
        # shape [batch_size, crop_h, crop_w, 3]
        cur_data = tf.compat.v1.nn.conv2d(HRdata, fix_gkern, strides=[1,4,4,1], padding="VALID", name='gauss_downsample_4')
...
def gif_summary(name, tensor, max_outputs, fps, collections=None, family=None):
    ...
    Returns:
      ...
      buffer.
    """
    tensor = tf.compat.v1.image.convert_image_dtype(tensor, dtype=tf.compat.v1.uint8, saturate=True)
    # tensor = tf.compat.v1.convert_to_tensor(tensor)
